...
/// Shared service for extracting text from various content types (HTML, rich text, plain text)
/// Generalized from EmailBodyExtractor to be used by all collectors
public struct TextExtractor {
    private let logger = HavenLogger(category: "text-extractor")
    
    public init() {}
    
    /// Extract text from content, converting HTML/rich text to markdown
    /// - Parameters:
    ///   - content: The raw content (HTML, rich text, or plain text)
    ///   - mimeType: Optional MIME type hint (e.g., "text/html", "text/plain")
    /// - Returns: Cleaned markdown text
    public func extractText(from content: String, mimeType: String? = nil) async -> String {
        // Determine if content is HTML
        let isHTML = mimeType?.lowercased().contains("html") == true ||
                    (content.contains("<") && content.contains(">") &&
                     (content.contains("<html") || content.contains("<body") ||
                      content.contains("<p>") || content.contains("<div") ||
                      content.contains("<br") || content.contains("<h1") ||
                      content.contains("<ul") || content.contains("<li")))
        
        // Convert HTML to markdown if needed
        let markdownText = isHTML ? await convertToMarkdown(content) : content
    ...
    /// Strip markdown blockquotes (they should be removed, not converted)
    ...
    /// Strip quoted content from text
