            ...
            Form {
                ...
                Section {
                    if !includeGlobs.isEmpty {
                        ForEach(Array(includeGlobs.enumerated()), id: \.offset) { index, _ in
                            HStack {
                        TextField("Pattern", text: Binding(
                            get: { includeGlobs[index] },
                            set: { includeGlobs[index] = $0 }
                ...
                Section {
                    if !excludeGlobs.isEmpty {
                        ForEach(Array(excludeGlobs.enumerated()), id: \.offset) { index, _ in
                            HStack {
                        TextField("Pattern", text: Binding(
                            get: { excludeGlobs[index] },
                            set: { excludeGlobs[index] = $0 }
