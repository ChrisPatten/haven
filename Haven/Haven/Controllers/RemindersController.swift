...
/// Controller for Reminders collector
public actor RemindersController: CollectorController {
    public let collectorId = "reminders"
    
    private let handler: RemindersHandler
    private let baseState: BaseCollectorController
    private let logger = HavenLogger(category: "reminders-controller")
    ...
    public init(
        ...
        // Submitter should be provided by HostAgentController (handles debug mode)
        // If not provided, create one (shouldn't happen in normal operation)
        let docSubmitter: any DocumentSubmitter
        if let providedSubmitter = submitter {
            docSubmitter = providedSubmitter
        } else {
            // Fallback: create submitter (should use HostAgentController's createDocumentSubmitter in practice)
            let submissionClient = GatewaySubmissionClient(
            ...
            let handlerProgress: ((Int, Int, Int, Int) -> Void)? = onProgress != nil ? { scanned, matched, submitted, skipped in
                Task { @MainActor in
                    let progress = JobProgress(
                        ...
                        submitted: submitted,
                        skipped: skipped,
                        currentPhase: "Processing reminders",
                        phaseProgress: nil
                    )
                    onProgress?(progress)
                }
    ...
    public func reset() async throws {
        ...
        if fm.fileExists(atPath: stateFile.path) {
            ...
            logger.info("Deleted Reminders state file", metadata: ["path": stateFile.path])
        }
