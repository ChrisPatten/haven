...
- `postgres` – `postgres:15`, volume `pg_data`, seeded via `schema/catalog_mvp.sql`.
...
- `collector` – optional profile running the iMessage collector against the gateway proxy.
...
3. Entrypoint dispatches on `$SERVICE` (gateway, catalog, search, embedding worker, collector).
...
- **Routes**:
  ...
  - `POST /v1/catalog/events` – streams ingestion payloads to catalog.
...
- **Ingestion**: `POST /v1/catalog/events` processes `CatalogEventsRequest` payloads, upserts threads/messages/chunks, and writes to `embed_index_state`.
...
- **Worker Loop**: `services/embedding_worker/worker.py` polls `embed_index_state` for `pending` rows, fetches chunk bodies, and encodes them with `SentenceTransformer` (`BAAI/bge-m3`).
- **Vector Storage**: Uses `qdrant-client` to upsert embeddings into the configured collection (default `haven_chunks`).
- **Error Handling**: Retries transient failures, logs errors, and leaves rows in `pending` state for later reprocessing.
...
- **embed_index_state** – Tracks per-chunk embedding lifecycle (`pending`, `ready`, `failed`).
...
| `CATALOG_ENDPOINT`, `COLLECTOR_POLL_INTERVAL`, `COLLECTOR_BATCH_SIZE`, `IMDESC_PATH`, `IMAGE_ENRICHMENT_CACHE`, `OLLAMA_HOST` | Collector | Control ingestion targets and enrichment behavior. |
| `WORKER_POLL_INTERVAL`, `WORKER_BATCH_SIZE` | Embedding worker | Tuning knobs for throughput vs. load. |
...
- Expand automated tests for catalog endpoints, embedding worker failure scenarios, and collector end-to-end flows.
